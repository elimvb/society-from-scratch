...
def glb_to_thor(object_path, output_dir, engine, annotations, save_obj, save_as_json=False):
    ...
    if source_poly_count > target_poly_count:
        ...
        """
        # PLANAR DECIMATION
        dec_mod_name = "Decimate_Planar"
        bpy.ops.object.modifier_add(type="DECIMATE")
        bpy.context.object.modifiers[-1].name = dec_mod_name
        bpy.context.object.modifiers[dec_mod_name].decimate_type = 'DISSOLVE'
        bpy.context.object.modifiers[dec_mod_name].angle_limit = math.radians(0)  # 10 degrees in radians
        bpy.ops.object.modifier_apply(modifier=dec_mod_name)

        dec_mod_name = "Triangulate"
